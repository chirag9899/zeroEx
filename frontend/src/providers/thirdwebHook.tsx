  ...
  const getUserOrder = async () => {
    try {
      ...
      const updatedContractOrders = orders.map(order => {
        return {
          ...order,
          sellToken: order.sellToken === '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE' ? 'ETH' : 'USDC',
          buyToken: order.buyToken === '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE' ? 'ETH' : 'USDC',
        };
      });
      const ordersFromContract: Order[] = updatedContractOrders.map(order => ({
      ...
      ordersFromContract.forEach((order) => {
        const index = updatedOrders.findIndex((localOrder) => {
          const isMatch = localOrder.user_address === order.user_address &&
          ...
          console.log({
            ...
            createdAtMatch: BigInt(localOrder.createdAt) === BigInt(order.createdAt)
          });
        ...
        if (index !== -1) {
          ...
          const fulfillmentPercentage = (localAmount * 100n) / contractAmountToTransfer;
          updatedOrders[index].fulfilledAmount = `${localAmount} / ${contractAmountToTransfer} (${fulfillmentPercentage}%)`;
